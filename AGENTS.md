...
canonical URL, generated by `scripts/generate-qr.mjs` and run as the last
step of `scripts/build.sh`, after `hugo build`/`--gc --minify`. This is
build-time, not a Cloudflare Pages Function: it keeps the site fully static
and doesn't introduce a `functions/` runtime just for this.

- The script reads `public/sitemap.xml` (Hugo's default sitemap output) to
  enumerate published pages — this already reflects any `-b`/baseURL
  override `scripts/build.sh` applies for preview deployments, and already
  excludes `draft: true` content the same way the rest of the build does, so
  the script needs no separate page-discovery or draft logic of its own.
- Each sitemap `<loc>` (e.g. `.../about.html`, `.../food/index.html`) is
  mapped back to the clean, extension-less URL Cloudflare Pages actually
  serves (see "URL structure" above) and to an output directory alongside
  the existing page output — `public/about/qr.png`, `public/food/qr.png` —
  so nothing under `public/` is overwritten.
- The QR itself uses error-correction level `H` (~30% redundancy) with
  `static/assets/android-chrome-192x192.png` composited at the centre at
  ~20% of the code's width on a small white pad, which stays comfortably
  inside the level-H budget. Verified scannable with a real decoder
  (`jsQR`), not just visually.
- Dependencies (`qrcode`, `jimp` — both pure JS, no native/image-library
  build step) live in `package.json`; `scripts/build.sh` runs `npm ci`
